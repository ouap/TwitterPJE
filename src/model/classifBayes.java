...
/**
 * Calcule le tableau de tous les mots presents dans une base d'apprentissage sans repetition (negatifs, positifs ou neutre) 
 * @param fichier le fichier de base
 * @param fichier2 la base d'apprentissage (negatifs, positifs ou neutre)
 * @return mots
 * @throws IOException
 */
public static String[] tabMotsMood(String fichier, String fichier2) throws IOException{
	int tab = 0;
	...
	for(int i=0;i<motsTweet.length;i++){
		...
		for(int j = 0;j<motsMood.length;j++){
			...
			if (motsMood[j].equals(motsTweet[i])){
				...
				if(classif == 0){
				probaMot = (nbMotsMood[j]+1)/(nbMotsTot[j]+nbMotsTotMood);
				} else {
...
public static int classifierBayes(String fichier, String tweet, int classif) throws IOException{
	...
	float probaNeutre = probaTweetMood(fichier,"neutres.csv",tweet,classif);
